...
begin
  ...
  try
    ...
    SL.Add('driverpath=.\sqlite3.dll'); // padrao: dll ao lado do exe
...
var
  CurrentPath, NewPath, DirNorm, Token: string;
begin
  Result := True;
  ...
  // Evita duplicar
  Token := ';' + LowerCase(DirNorm) + ';';
  if Pos(Token, ';' + LowerCase(StringReplace(CurrentPath, '/', '\', [rfReplaceAll])) + ';') = 0 then
  begin
    if CurrentPath <> '' then
...
begin
  ...
  try
    ...
    Conn.LibraryLocation := ExtractFilePath(DriverPath);    // PASTA da DLL
